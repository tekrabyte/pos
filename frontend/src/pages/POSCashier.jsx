...
const POSCashier = () => {
  ...
  const handleBarcodeScanned = (sku) => {
  ...
  const completeTransaction = async () => {
    try {
      ...
      const orderForReceipt = {
        order_number: orderResponse?.data?.order_number || `ORD-${Date.now()}`,
        payment_method: paymentMethod,
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Receipt-${lastOrder?.order_number || 'XXXX'}`,
  });
    ...
    <div className="flex h-screen bg-gray-50" data-testid="pos-cashier-page">
      ...
      <div className="hidden">
        <ReceiptPrint
          ref={receiptRef}
          orderData={lastOrder}
          cart={cart.length > 0 ? cart : JSON.parse(localStorage.getItem('pos_cart') || '[]')}
        ...
        <DialogContent className="max-w-md" data-testid="payment-dialog">
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <CreditCard className="h-6 w-6 mb-2 mx-auto text-blue-600" />
              ...
              >
                <CreditCard className="h-6 w-6 mb-2 mx-auto text-green-600" />
                <p className="font-medium text-sm">QRIS</p>
              </button>
            </div>

            {showQRIS && qrisData && (
              <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg text-center border-2 border-blue-200">
                <p className="text-sm font-medium text-gray-700 mb-4">Scan QRIS untuk membayar</p>
                <div className="bg-white p-4 rounded-lg inline-block shadow-md">
                  <QRCodeSVG
                    value={qrisData.qris_string}
                    size={200}
                    level="M"
                    includeMargin
                  />
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Merchant ID: {qrisData.merchant_id}
                </p>
              </div>
            )}
